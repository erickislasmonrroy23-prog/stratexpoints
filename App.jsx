...
function MainApp({ onLogout, onSuperAdmin }){
  ...
  // ("Maximum update depth exceeded") causado por la creación de nuevos objetos en el selector. Al usar `shallow`
  // evitamos re-renders si el contenido es el mismo, aunque la referencia haya cambiado.
  const { alerts, objectives, realProfile, user, impersonatedProfile } = useStore(state => ({
    alerts: state.alerts,
  ...
  // COMMENT (OPTIMIZATION): Se centraliza la lógica de guardado, notificación y cierre de modal.
  // Esto reduce la duplicación de código de los manejadores `handleSaveOKR`, `handleSaveKPI`, etc.
  // El siguiente paso sería mover esta lógica a una acción del store de Zustand.
  const handleGenericSave = async (savePromise, successMessage) => {
  ...
  // COMMENT (CLEANUP): Las funciones `handleSaveOKR`, `handleSaveKPI` y `handleSaveInitiative` han sido eliminadas
  // y reemplazadas por llamadas en línea al manejador genérico `handleGenericSave`.
  // Esto elimina más de 40 líneas de código repetitivo y centraliza el manejo de errores y estado de UI.
  ...
  return(
    ...
    <ErrorBoundary key={activeModule}>
      <Suspense fallback={<ModuleSkeleton />}>
              ...
              {activeModule==="okrs"&&<ModuloOKRs onModal={() => {setEditingItem(null); setModal("okr");}} onEdit={(item) => {setEditingItem(item); setModal("okr");}} onDelete={handleDeleteOKR} />}
              {activeModule==="kpis"&&<ModuloKPIs onModal={() => {setEditingItem(null); setModal("kpi");}} onEdit={(item) => {setEditingItem(item); setModal("kpi");}} onDelete={handleDeleteKPI} onCreateOkrFromKpi={function(kpi){
              ...
              {activeModule==="iniciativas"&&<ModuloIniciativas onModal={() => {setEditingItem(null); setModal("initiative");}} onEdit={(item) => {setEditingItem(item); setModal("initiative");}} onDelete={handleDeleteInitiative} />}
      ...
      {/* COMMENT (OPTIMIZATION): La lógica de guardado se pasa como una función anónima que invoca al manejador genérico.
          Esto mantiene la lógica de negocio fuera del componente del formulario, pero elimina la necesidad de tener
          funciones `handleSave...` separadas y voluminosas en `MainApp`. */}
      {modal==="okr"&&<Modal onClose={onModalClose}><OKRForm onSave={(form) => {
        const { id, ...payload } = form;
        if (!payload.objective_id) payload.objective_id = null;
        const promise = id ? okrService.update(id, payload) : okrService.create(payload);
        handleGenericSave(promise, `OKR ${id ? 'actualizado' : 'creado'} exitosamente.`);
      }} onCancel={onModalClose} objectives={objectives || []} initialData={editingItem}/></Modal>}
      {modal==="kpi"&&<Modal onClose={onModalClose}><KPIForm onSave={(form) => {
        const { id, ...payload } = form;
        const promise = id ? kpiService.update(id, payload) : kpiService.create(payload);
        handleGenericSave(promise, `KPI ${id ? 'actualizado' : 'creado'} exitosamente.`);
      }} onCancel={onModalClose} initialData={editingItem}/></Modal>}
      {modal==="initiative"&&<Modal onClose={onModalClose}><InitiativeForm onSave={(form) => {
        const { id, ...payload } = form;
        const promise = id ? initiativeService.update(id, payload) : initiativeService.create(payload);
        handleGenericSave(promise, `Iniciativa ${id ? 'actualizada' : 'creada'} exitosamente.`);
      }} onCancel={onModalClose} initialData={editingItem}/></Modal>}
