...
export default function ChangePassword() {
  ...
  const validatePassword = (password) => {
    ...
    if (password.length < 12) {
      errors.length = 'La contraseña debe tener al menos 12 caracteres';
    }
    if (!/[A-Z]/.test(password)) {
      errors.uppercase = 'Debe contener al menos una letra mayúscula';
    }
    if (!/[0-9]/.test(password)) {
      errors.numbers = 'Debe contener al menos un número';
    }
    if (!/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(password)) {
      errors.symbols = 'Debe contener al menos un carácter especial (!@#$%^&*)';
    }
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      ...
      // Update password_rotation_due flag in profiles table
      ...
      if (dbError) {
        logger.error('Error updating password rotation flag:', dbError);
        // Don't fail here, password was already updated
      }
          ...
          {[0, 1, 2, 3].map((i) => (
            <div
              ...
              style={{
                ...
                height: 6,
                borderRadius: 3,
                background: i < score ? colors[i] : 'var(--bg3)',
                transition: 'all 0.2s',
              }}
            />
          ))}
  ...
  const PasswordInput = ({ label, value, onChange, show, onToggleShow, error, name }) => (
