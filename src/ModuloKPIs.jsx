...
export default function ModuloKPIs({ onModal, onEdit, onCreateOkrFromKpi, onDelete }) {
  ...
  const renameDept = async (oldName, newName) => {
    ...
    try {
      await Promise.all(
        (kpis || [])
          .filter(o => (o.department || 'General') === oldName)
          .map(o => kpiService.update(o.id, { department: newName }))
      );
  ...
  return (
    <div>
      ...
      <TabBar
        ...
        rightContent={
          tab === 'list' && can('create', 'kpis')
            ? <AddBtn onClick={() => setShowKpiModal(true)} label="Nuevo KPI" color="var(--teal)" />
            : null
        }
      />
      ...
      {/* ── Modal Crear KPI — vive aquí en el componente padre ── */}
      {showKpiModal && (
          ...
          <div className="sp-card" style={{ width: '100%', maxWidth: 520, padding: 32, borderRadius: 20, boxShadow: '0 24px 48px rgba(0,0,0,0.2)' }}>
            ...
            <form onSubmit={handleCreateKPI} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
                ...
                <button type="button" onClick={() => setShowKpiModal(false)} className="sp-btn" style={{ flex: 1, padding: '12px', borderRadius: 10, fontSize: 14 }}>
                  Cancelar
                </button>
              </div>
            </form>
    ...
    if ((kpis || []).length === 0) {
      return (
        <EmptyState
          ...
          action={can('create', 'kpis') ? <AddBtn onClick={() => setShowKpiModal(true)} label="Crear KPI" color="var(--teal)" /> : null}
        />
      );
    }
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap', background: 'var(--bg2)', padding: 16, borderRadius: 12, border: '1px solid var(--border)' }}>
          ...
          {(filterOwner !== '' || filterDept !== '') && (
            <div style={{ display: 'flex', alignItems: 'flex-end' }}>
              <button className="sp-btn" onClick={() => { setFilterOwner(''); setFilterDept(''); }} style={{ background: 'var(--bg3)', color: 'var(--text2)', border: '1px solid var(--border)', height: 38 }}>
                Limpiar
              </button>
            </div>
          )}
...
function DeptSection({ dept, kpis, collapsed, onToggle, editingDept, onEditDept, onRenameDept, can, onEdit, onDelete, onCreateOkrFromKpi, getColor }) {
  return (
    <div style={{ marginBottom: 8 }}>
      <div
        onClick={e => { if (e.target.tagName !== 'INPUT' && e.target.tagName !== 'BUTTON') onToggle(dept); }}
        className="sp-card-hover"
        style={{ padding: '16px 24px', background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: collapsed ? 0 : 16 }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {editingDept === dept ? (
            <input
              ...
              onKeyDown={e => { if (e.key === 'Enter') e.target.blur(); if (e.key === 'Escape') onEditDept(null); }}
            ...
            <h4 style={{ fontSize: 16, fontWeight: 800, color: 'var(--text)', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
              {dept}
              {can('update', 'kpis') && (
                <button onClick={e => { e.stopPropagation(); onEditDept(dept); }} className="icon-btn" style={{ width: 24, height: 24, fontSize: 12, border: 'none', background: 'transparent', opacity: 0.5 }}>✏️</button>
              )}
            </h4>
          )}
        ...
        <span style={{ transform: collapsed ? 'rotate(-90deg)' : 'rotate(0deg)', transition: 'transform 0.2s ease', fontSize: 14, color: 'var(--text3)', fontWeight: 800 }}>▼</span>
      </div>
      ...
      {!collapsed && (
        <div className="scale-in" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 16, paddingLeft: 28, borderLeft: '2px dashed var(--border)', marginLeft: 28, marginBottom: 32 }}>
          {kpis.map(kpi => (
            <KpiCard key={kpi.id} kpi={kpi} can={can} onEdit={onEdit} onDelete={onDelete} onCreateOkrFromKpi={onCreateOkrFromKpi} getColor={getColor} />
          ))}
        </div>
      )}
    </div>
...
// ── Tarjeta KPI — sin estado global, sin modales ────────────────────────────
    ...
    <div className="sp-card sp-card-hover" style={{ padding: 20, display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {can('update', 'kpis') && (
            <button className="icon-btn" onClick={() => onEdit && onEdit(kpi)} style={{ width: 28, height: 28, fontSize: 13, background: 'transparent', border: '1px solid var(--border)', color: 'var(--text3)' }}>✏️</button>
          )}
          {can('delete', 'kpis') && (
            <button className="delete-btn" onClick={() => onDelete && onDelete(kpi.id)}>🗑</button>
          )}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 16, display: 'flex', alignItems: 'flex-end', gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'baseline' }}>
          <input
            ...
            style={{ width: 70, fontSize: 32, fontWeight: 800, color, background: 'transparent', border: 'none', borderBottom: '1px dashed transparent', outline: 'none', padding: 0, cursor: can('update', 'kpis') ? 'text' : 'not-allowed', lineHeight: 1, opacity: can('update', 'kpis') ? 1 : 0.6 }}
          />
          <span style={{ fontSize: 16, color: 'var(--text3)', marginLeft: 2, fontWeight: 700 }}>{kpi.unit}</span>
        </div>
      ...
      {pct < 80 && can('create', 'okrs') && (
        <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
          <div style={{ fontSize: 11, color: 'var(--red)', marginBottom: 8, fontWeight: 700 }}>Desviacion detectada</div>
          <button className="sp-btn" onClick={() => onCreateOkrFromKpi && onCreateOkrFromKpi(kpi)} style={{ background: 'var(--red-light)', color: 'var(--red)', border: '1px solid rgba(220,38,38,0.3)', width: '100%', justifyContent: 'center', fontSize: 12 }}>
            Crear OKR de Mejora
          </button>
        </div>
      )}
