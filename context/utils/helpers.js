...
export const aiService = {
  ...
  callJSON: async (prompt, system, maxTokens) => {
    ...
    catch { throw new Error("La IA no devolvió JSON válido"); }
  },

  /**
   * Genera OKRs para un objetivo estratégico
   */
  generateOKRs: (objective, sector, period) =>
    aiService.callJSON(
      `Genera 2 OKRs SMART para el objetivo estratégico: "${objective}" en el sector ${sector}, período ${period}.
       JSON: {"okrs":[{"objective":"string","keyResults":[{"text":"string","baseline":number,"target":number,"unit":"string","type":"LEADING|LAGGING"}]}]}`,
      "Master Black Belt OKRs y Hoshin Kanri. Solo JSON válido.",
      800
    ),
   ...
   */
  executiveDiagnostic: (okrs, kpis, avgProgress) =>
    aiService.callJSON(
      `Diagnóstico ejecutivo. OKRs: ${JSON.stringify(okrs.slice(0,4))}. KPIs críticos: ${JSON.stringify(kpis.filter(k=>k.trafficLight==="red"))}. Avance global: ${avgProgress}%.
       JSON: {"resumen":"string","ieg":number,"top3":["string","string","string"],"riesgos":["string"],"mensaje_directorio":"string"}`,
      "CSO virtual. Análisis ejecutivo conciso en español.",
      1000
    ),
  ...
  analyzeDocument: (text, filename) =>
    aiService.call(
      `Analiza este documento estratégico "${filename}" y extrae:
       1) Tipo de documento
       2) Objetivos estratégicos identificados
       3) KPIs y métricas mencionadas
       4) Responsables clave
       5) Períodos y fechas importantes
       6) Riesgos identificados
       7) Recomendaciones estratégicas

       Documento:
       ${text.substring(0, 4000)}`,
      "Analista estratégico senior. Respuesta estructurada en español.",
      1200
    ),
};
...
export const validate = {
  ...
  isPercent: (v) => parseFloat(v) >= 0 && parseFloat(v) <= 100,
  isDate:    (v) => !isNaN(new Date(v).getTime()),
  isEmail:   (v) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v),

  okr: (data) => {
    const errors = {};
    if (!validate.required(data.objective)) errors.objective = "El objetivo es requerido";
    if (!validate.required(data.owner))     errors.owner     = "El responsable es requerido";
    if (!validate.required(data.period))    errors.period    = "El período es requerido";
    return { valid: !Object.keys(errors).length, errors };
  },

  kpi: (data) => {
    const errors = {};
    if (!validate.required(data.name))        errors.name   = "El nombre es requerido";
    if (!validate.isNumber(data.target))      errors.target = "La meta debe ser un número";
    if (!validate.isPositive(data.target))    errors.target = "La meta debe ser positiva";
    return { valid: !Object.keys(errors).length, errors };
  },
};
